...
### 2.11 微轮：门禁现状同步（2026-09-19 · eng-designer）

**范围**：仅 `docs/CONVENTIONS.md`（B16 §2.8 待同步项「规范档侧收口」）。未碰任何其它档。

**改动清单（4+1 处，全部落地）**：
1. §零 摘要表「门禁」行（现 L20）：计数 **0/3 → 3/3**（其余字面与指针不动）。
2. §四 结构判据三条行（现 L113）：「机检随 B16 实施落地」→「机检**已落地**（2026-09-19）」（设计/基线契约指针保留）。
3. §四 基线处理行（现 L122）：「（随 B16 实施落地后、`scripts/gates/baseline.json` 生效）」→「（已落地生效：`scripts/gates/baseline.json`——冻结 + 只拦新增 + 陈腐即红）」。
4. §五 行宽债现状行（现 L136）：计数 **92 行 / 16 档 → 15 档 / 93 行**（与 `scripts/gates/baseline.json` 对齐：15 档合计 93 行，逐档相加核验 ✓）；口径注改「基线冻结面；计数权威 = `scripts/gates/baseline.json`」；as-of 保留 2026-09-18（= baseline.json 的 asOf）；「与 B07 期 65 行并存」句保留。
   - **连带一致性修正**：「余 13 档」→「余 12 档」（15 - 前三 3 = 12；D3——原 13 与 16 档匹配、与新 15 档不符）。
   - 原「口径 = 不含行尾 CR >300；豁免面 = 台账/地图表格行 + 批次档 §3」枚举移除——两处已在 §五 表格行（L132）与门禁句（L138）详述（D2 单一权威源）；本行新口径注 = 基线冻结面 + 计数权威。行宽实测 **299 字符**（≤300；原行 300 贴线，不压缩必超）。
5. §五 门禁句尾行（现 L139）：「机检随 B16 实施落地」→「机检**已落地**」（同口径）。
6. §变更记录 += 1 行（2026-09-19，现 L191）：门禁计数 0/3 → 3/3 · 「随 B16 实施落地」→「已落地」×2 · 基线处理行改「已落地生效」· 行宽债计数与 baseline.json 对齐（92 行 / 16 档 → 15 档 / 93 行；余 13 → 12 档同步）；依据 = `docs/batches/B16-test-gates.md` §5 / §6。

**自检（D6 回读）**：7 处逐行回读核对 ✓；`docs/CONVENTIONS.md` 全档无 >300 字符行 ✓；「随 B16 实施落地」零残留（变更记录历史行除外——历史语义保留）。lint 结果见下（本段写于 lint 之前）。

**lint 结果（2026-09-19 实测）**：`npm run lint` → `GATE lint PASS checks=6 selftest=14/14`（`CHECK width PASS frozen=15 lines=93`——冻结面与 `baseline.json` 一致，新增违规 0）。
**修正轨迹（如实）**：首跑红 1 = 本段第 4 条长行（487 字符，批次档 §2 非豁免面）——已就地拆为 3 行（≤300）后复跑绿；第 4 条语义未变、仅折行。§3 既有 3 条 >300 行在豁免面内，未动。
