...
- 防复发（三条）：① 刷新**幂等**（已最新时为空操作）；② 判定**可查**（只读检查的退出码语义 = 已最新 / 落后 / 取版本失败）；③ 刷新失败**不留半成品**（钉版回滚、锁文件不自洽即报错）。
...
- 锁文件 `dsh-bundle/package-lock.json` **随仓**，且与其 `package.json` 自洽：`npm ci`（或等价校验）能按锁文件复现同一棵树；不自洽即失败态。
- 「可复现」的语义 = **同一份锁文件复现同一棵树**；跨刷新时刻的树不承诺逐字节可比（精确钉版只钉主包，传递依赖由 semver 范围与注册表当下状态决定）。
- 刷新动作不经构建流程隐式触发；刷新产物（钉版 + 锁文件）随提交入仓，可由 `git diff` 审阅。
- 度量方式：钉版形态检查（范围符 / 标签 0 命中）；`npm ci`（或等价校验）退出码；刷新脚本的判定退出码与摘要行。
