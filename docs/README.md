...
| `docs/requirements/SHELL.md` | 需求档 | 生效（B06：评审通过，待用户批准） | 桌面壳板块需求：启动形态 / 托盘 / 窗口 / 桌宠交互入口 / 更新门禁 / `main.js` 拆分 / 无用文件清理（US-1…US-8、NFR-1…NFR-4；B06） |
...
| `docs/design/SHELL-UX.md` | 设计档 | 已评审通过（待用户批准） | B06 桌面壳设计：需求层回指（US-1…US-8、NFR-1…NFR-4） / 选型 A–F / 契约与结构（含 15 模块拆分与迁移） / 受影响文件全清单 / DD-1…DD-16 / 观察项与限制 / U-1…U-10 / AC1–AC8 + TC-1…TC-26；评审 2 轮（轮次 1 changes-required → 修正轮 1 → 轮次 2 pass） |
...
| `docs/batches/B06-shell-ux.md` | 批次档 | 已评审通过（待用户批准） | B06 批次六段记录（桌面壳 UX 整合：启动形态 / 桌宠左右键语义 / 删新手向导 / 托盘重排 / 更新门禁口径 / `main.js` 拆分 / 无用文件清理（F7））；需求档 `docs/requirements/SHELL.md`、设计档 `docs/design/SHELL-UX.md` 已新建；§3 待评审写入 |
| `docs/TODO.md` | 台账 | 生效 | 需求池（R1 / R2 / R4 / R5 / R6 / R7，6 条；R3 已核销、在归档档）与技术待办（T1 / T2 / T4 / T5 / T7 / T8 / T9 / T10，8 条；T3 / T6 已核销、在归档档）：一条一行，挂需求档节 + 批次档 §2 / 归属档节 + 最小证据行；归档档 = `docs/TODO-archive.md` |
