...
| T13 | **行宽债清理（非豁免面实测 65 行——修正原登记的 3 行）**：明细 = `B03-pet-multimonitor.md` **40**（§2 3 / §5 37）· `PET-MULTIMONITOR.md` **18** · `AUTO-UPDATE.md` **3**（`:57` 366 / `:796` 309 / `:993` 321；行号 as-of 已漂移，原记 `:53/:698/:853`）· `B01` §2 1 · `B05` §6 1 · `B06` §5 1 · `PET.md:214` 1（321，原未登记）；**豁免面**另 56 行（批次档 §3 34 / 台账 16 / 地图 6）不动。①②③（跨档一致性）已由 B07 收口 | 见上各档 | 主 agent 2026-09-17 全仓实测（分段归属 + 不含 CR 口径）；与 B11 设计者独立计数一致（65） | 待讨论 | 条件（**消解路径**：**活文档 22 行**（`PET-MULTIMONITOR` 18 + `AUTO-UPDATE` 3 + `PET.md` 1）随 **B14** 修正；**批次档历史段 43 行**随该档下次被合法动笔时一并压行——父侧不代笔改写他人段；用户若要求立即清，另批授权） |
