...
| US-21 工作状态只作「底色」（B19；**B26 追加自主触发让位**） | §2.8.4 / **§2.12** / §3.4 AC19、AC21、**AC24** | 工作档 = 背底档 + 1 s 重断言；散步与自主小动作的**起步**各一道门（同口径）；既有触发点与定时器逐字不改 |
...
| 5 | 无在途回合 ∧ 记录新鲜（`now − mtime ≤ WORK_STALE_MS`）∧ `turnBoundary.val.lastTurn > doneBaseline` | `work-done`（持续 `WORK_DONE_HOLD_MS` = **3000**；触发即置 `doneBaseline = lastTurn`） |
| 6 | 其余（长期空闲 / 陈旧记录 / 收尾保持期满） | **清档**（回 idle） |

- **次序声明**：判定自上而下短路；规则 2 先于 3——工具在跑时 `openStep` 通常已清空，但并行调用窗口内二者可同时在场 ⇒ **工具优先**（「在干活」比「在思考」更准）；
- **常量（`pet-work-core.js`，可被桩测逐值断言）**：`WORK_TICK_MS = 1000` · `WORK_STALE_MS = 60000` · `WORK_DONE_HOLD_MS = 3000` · `WORK_BUBBLE_MIN_GAP_MS = 30000`（`WORK_DONE_WINDOW_MS` 随规则 5 改写**删除**——不再有消费者，死常量不留）；
...
- **收尾保持期（3 s）与陈旧面的分工**：① 保持期 = 触发后 `WORK_DONE_HOLD_MS`（3000）内不落规则 6——期内回合重新在途 ⇒ 立即退出保持、转规则 2 / 3；期满 ⇒ 规则 6 清档；② 陈旧守卫（§2.8.1：回合在途 ∧ mtime 超 60 s ⇒ 按无在途回合处理）与规则 5 的「记录新鲜」条件合读 ⇒ **陈旧记录不进收尾档**、直接落规则 6 清档（= TC-31 的期望；旧写法下「陈旧也演一次收尾」的相抵面随本次改写一并消除）；
...
**注 D —— 自主触发的让位（B26 / F1 / US-21）**

- 优先级表与共同门的**口径逐字不变**；B26 新增的只是**起步门**：待机随机小动作（`read` / `starry` / `scared` / `happy` 四档）在工作档在途期间**不启动**，与散步起步门**同一判据形态**（`petState === 'idle' && petBaseState() === 'idle'`）。
...
- **单候选收尾档**（`work-done`；单候选 ⇒ `loop=true`，`pet-chain.js:133-134`）：**不受该守卫保护** ⇒ `WORK_DONE_HOLD_MS`（3 s）到点即切（素材长于 3 s ⇒ 切在 3 s 处；短于 3 s ⇒ 循环续播至 3 s 止）；观感取舍登记 = §2.11 观察项 O19；
...
- **O19 收尾档的可见时长受 3 s 保持期限制（修正轮 1 #12 发现；观感取舍，建议批准时复核）**：`work-done` 为单候选 ⇒ `loop=true` ⇒ `WORK_DONE_HOLD_MS`（3 s）到点即切；若 `工作状态-清点归档` 素材时长 > 3 s ⇒ 观感为**中途切断**（非播完）。两处可调：① 提高 `WORK_DONE_HOLD_MS`（须与既有「任务完成」提醒的节奏同看）；② 把该档改为多候选（`loop=false` 即转为「播完才回链」）。本批按 3 s 成文（承 DD-19 / TC-22），取值复核归用户批准面。

**B26 面新增（F1 / F3 相关；发现即报告）**

- **O20 F1 的效力边界（读面清档间隙；本批不做，登记）**：F1 的起步门以**已下发的工作档**（`petBaseState()` = `lastGear`）为判据 ⇒ 在「读面清档 / 检测间隙」内（`deriveGear` 规则 6 的 idle 期 + 写节流 5 s 造成的回合起始延迟）它**封不住**自主动作（`lastGear` 已是 `null`）。
  要覆盖这段，需一个比「工作档」更宽的**「会话活跃」信号**（例：选中记录新鲜，`now − mtime ≤ WORK_STALE_MS`）——属**需求层新口径**（会把「会话开着但模型空闲」的时段也算作让位期），**本批不自作**。
  **实测补充（供裁定参考）**：自主动作可达的窗口 = ① 上条的读面间隙（**射程外**）；② 拖动在途（`gateBlocked` 让位、`lastGear` 未更新）与 ③ 交互档结束 → 重断言之间的 ≤1 s（**②③ 在射程内**，本门直接生效）。
- **O21 F3（「工具在跑」档补气泡）的前提与实现不符 ⇒ 打回（需求层口径待裁）**：
  ① **文案已在位**——`WORK_QUOTES` 已含 `work-working` 两条（`pet-work-core.js:23-26`）；② **状态机已共用**——`pickBubble()` 按档查键、`applyTick()` 每 tick 调同一 `bubbleState`（无第二套）；③ **真因 = 节流口径**——`bubbleState.lastAt` 是**跨档全局**计数器（`shell-pet-work.js:35`）⇒ 生成档进入时必弹 ⇒ 工具档在 < 30 s 内进入时被同一窗口压掉；
  ④ **量化（对真实 `pickBubble` 跑合成时间线）**：思考 20 s / 工具 5 s 交替 ⇒ 30 min 内气泡**全为生成档**；思考 45 s / 工具 30 s ⇒ 两档各 2 条 ⇒ 工具档**只在自身持续 ≳30 s 时**才可能被报到；⑤ 次要面：让位 / 无窗口时 `pickBubble` **也推进 `lastAt`**（`applyTick` 的调用次序在让位 return 之前）⇒ 被吞掉的那次同样占掉窗口。
  **性质 = 语义面（需求口径）**：要交付「工具档可见」，须把 `WORK_BUBBLE_MIN_GAP_MS` 的作用域由**跨档全局**改为**按档**；US-22 的「两条工作气泡间隔 ≥ 30 s」字面支持**两种读法**（全局 / 同档），结论相反（不可交付 / 可交付）⇒ **停下打回主 agent，不自行选一种往下写**（两条读法与建议 = 批次档 `docs/batches/B26-pet-feel.md` §2）。**F3 未纳入本批**。
  另注：**自主闲聊气泡与工作气泡共用同一气泡元素**（后到者覆盖）——若日后把闲聊也纳入让位，须与该裁定同轮处置。

---

### 2.12 B26 面：自主触发让位（F1，US-21）

> 面 = 「工作档在途期间，**自主触发**的表演性动作与散步**同口径**让位」；承 US-21 的既有口径（「工作档在途期间不启动散步」+ §2.8.4 优先级表）与台账 R19 / 批次档 `docs/batches/B26-pet-feel.md` §1.4 F1。
> 变更面 = **一道起步门（1 行）**；优先级表 / 共同门 / 既有触发点与定时器**逐字不变**（与 §2.8.4 **注 D** 同源）。

#### 2.12.1 方案选型对比（门位；对应批次档 §1.7 U-1）

| # | 候选方案 | 判据逐项评估 | 取舍（选定代价 / 权衡） | 结论 |
|---|---|---|---|---|
| 1 | **起动侧门**：`playIdleVariant()` 首行守卫加 `petBaseState() === 'idle'` | 与散步起步门**同判据形态、同常量来源**：✅ · 改动 = 1 行：✅ · 保护全部调用方（含将来新增）：✅ · 共同门零改：✅ · 台词面零改（US-21「台词照旧」）：✅ | 代价 = 无（口径对齐）；**效力边界**见 §2.12.3 | **选定**（DD-27） |
| 2 | **共同门侧排除**：在 `shell-pet-work.js` 的 `gateBlocked()` 里区分「自主 / 用户触发」 | 需给档位附加「触发来源」= 新跨模块状态 ❌ · **不阻止**自主动作起步（仅取消让位）⇒ 工作档与交互档同时争动画档 ⇒ 档位乒乓 ❌ · 与 `getPetState()` 的只读面不兼容 ❌ | — | **否决**——治标不治本 + 新增双源 |
| 3 | 摘除自主小动作（从 `schedulePetChatter` 移除 `playIdleVariant()` 调用） | 删除既有可见行为（US-21 边界「既有交互与观感照旧」）❌ · 超出本面范围（要的是「工作档期间让位」，不是取消）❌ | — | **否决** |

#### 2.12.2 门位契约（判据句）

- **自主随机小动作的启动条件**（`shell-pet.js` 的 `playIdleVariant()` 首行守卫）：`petWindow 在场 ∧ petState === 'idle'` **∧** `petBaseState() === 'idle'`；任一不满足 ⇒ 直接返回（不 `setPetState`、不排队、不重试、零新定时器）。
...
- **保留面（逐字不改）**：点击（`handlePetClicked` → `happy` + 台词）· 喂食（`shell-affinity.handleAffinityBuy` → `eat`）· 拖动（`shell-pet-drag.js`）· 闲聊台词（`schedulePetChatter` 的 `petSay`：90 s 节拍与 40% 概率）· 小动作时长（2.4 s）与档位名（11 档词表）。
- **与共同门的关系（D2）**：共同门（§2.8.4）**不改**——它判的是「本 tick 是否下发 / 重断言」（对**已在途**的交互档让位）；本门判的是「**新的**自主动作是否起步」。两者互补、不重叠（注 D）。
- **边界（不做）**：不新增日志行型 / 开关 / IPC / 档位名；不动 `main.js` 注入面（零新增依赖边）；**不改自主台词**（见 §2.11 观察项 O21 的尾注）。

#### 2.12.3 效力边界与残余（如实登记）

- **本门关闭的窗口**：① 拖动在途（`gateBlocked` 让位期、`lastGear` 未更新）；② 「交互档结束 → 工作档重断言」之间的 ≤1 s——两处均在射程内。
- **本门覆盖不到的窗口（= §2.11 观察项 O20）**：读面**清档 / 检测间隙**（`deriveGear` 规则 6 的 idle 期 + 写节流 5 s 造成的回合起始延迟）内 `petBaseState()` 已是 `idle` ⇒ 自主动作仍可起步；随后的让位（≤2.4 s）与工作档恢复（≤1 s）与今天同形。要覆盖这段需要更宽的「会话活跃」信号（属需求层新口径，本批不自作）。
...
| # | 决策 | 理由 | 否决 / 备选 |
|---|---|---|---|
| **DD-27** | **门位 = 自主动作自身（`playIdleVariant()`），而非其调度器或共同门** | ① 与散步起步门同形同源（口径一致）；② 只闸「动作」不闸「台词」——`schedulePetChatter` 的回调体同时承载闲聊台词（US-21 明列「台词照旧」+ 保护 1.5 min 参数）⇒ 门必须落在**动作入口**，不能落在回调层；③ 保护全部调用方（含将来新增） | 调度器层加门（会连带闸掉台词）· 共同门侧排除（§2.12.1 候选 2） |
...
| **AC24** | US-21（B26） | **自主触发让位（起步门）**：① 符号级——`playIdleVariant()` 体内含 `petBaseState()` 命中 ≥1，且在任何 `setPetState` **之前**（与 `scheduleWander` 起步门**同形**）；② 运行时（视频通道）——工作档在途的日志里**无** `anim slot key=events.read|starry|scared|happy` 行；③ 静态——90 s / 40% 与 2.4 s 逐字未改；④ 保留面 = TC-35 | 静态机检 + 日志面 |
...
| **TC-34** | 正常 | 工作档在途（`work-thinking` / `work-working`）持续 ≥ 5 min（无用户输入） | 日志窗口内**无** `anim slot key=events.read|starry|scared|happy` 行（自主小动作未起步）；工作档自身的 `anim slot` / `work reassert` 行照常 | US-21 / AC24 |
| **TC-35** | 边界 | 工作档在途时：① 点击领鱼娘；② 喂食一次 | 两者仍压过工作档（`anim slot key=events.happy` / `events.eat` 行在场 + `work reassert` 行在交互档结束后 ≤1 s 出现）——**自主门不涉用户输入触发** | US-21 / AC24 |
| **TC-36** | 异常 | 工作档干净地不在途（`petBaseState()` = `null`）且无其它阻挡 | 自主小动作照旧可起步（`anim slot key=events.read|starry|scared|happy` 行在场）——门只封工作档在途之情形，**不把功能关掉** | US-21 / AC24 |
