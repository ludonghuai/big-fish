...
| # | 验收标准（回指 §1） | 判据（可机检） |
|---|---|---|
| AC-B16-1 | **`lint` 门在场且本地可跑**（§1.4-1） | `npm run lint` 退出 0 且 stdout 含 `GATE lint PASS checks=6 selftest=14/14`（串 = A.2.2.1 契约表的摘要行，二者同源）；`package.json` scripts 的 `lint` = `node scripts/gates/run.js`（A.2.2.1 字面值表） |
