...
- **调值 / 再测路径**：`FEET_ALPHA_MARGIN_DIP` 一处（4 → **0** 为**回正** = 只对齐锚点、全通道不越沉；4 → 其他值 = 细调）；改值须同步 §2.2.3 表值、§2.2.13 常量块与 §3.1 AC21① / 注 A 的等式解释。
  再测 = `npx electron scripts/probes/probe-pet-media.js`（打 alpha 包围盒）/ PNG alpha 解码（逐段）。
...
| 2026-09-20 | **B15 实施后文档同步轮**：O-13 细目再测命令 → `npx electron scripts/probes/probe-pet-media.js`（B15 归位）。**判据 / 计数零改动。** 依据 = `docs/batches/B15-repo-hygiene.md` §5。 |
