...
| R24 | 右键弹窗重做 + 动作图鉴卡片化（用户 2026-09-20 提出；四条裁定 D-1…D-4） | **已核销**——B35 全链闭环：设计评审两轮（轮 1 CR 🔴2/🟡5/🔵4 → 修正 11/11 → 轮 2 pass）→ 用户批准 → 实施（三道门全绿 + 桩测 101/101 + 冻结面零 diff）→ §6 核销（机检全绿）→ 实机微修 ×3 → **用户实机目视通过 ✓**（2026-09-20 原话「B35可以了，我都看过」）；证据 = `docs/batches/B35-pet-gallery-ui.md` §3–§6 | B35 | 2026-09-20 |
