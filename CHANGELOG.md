...
（本节收下一版变更；条目来源 = 各批次档 §6 收口行）

## [0.0.2] — 2026-09-20 发布（GitHub release `v0.0.2`；清单 = Gitee raw `latest.json`）

### Added

- 桌宠动作解锁与图鉴（新功能）：节日 / 饭点 / 等级**三把锁**——到点或到级自动解锁对应动作（**默认全开**；开关关掉 ⇒ 回到「随时可出」）；交换窗新增「**图鉴**」标签卡（动作 / 解锁条件 / 解锁百分比；未解锁显名灰化，共 80 行）——`docs/batches/B23-pet-action-unlock.md`
- 右键弹窗重做与动作图鉴卡片化（新功能）：兑换窗**可调整大小**（默认 720×560、记住尺寸）· 喂养面重排（头部卡 + 食品卡）；图鉴升级**视频卡片墙**（每个动作一张卡、可见即播 / 离屏即停、顶部收集进度）；图鉴内可**喜欢**动作（♥ 星标、更常出现、置顶成组、「只看喜欢」开关）与**屏蔽**动作（随机池与事件触发都不再出现；拖动 / 逃跑 / 待机三个事件独占段不可屏蔽）；**满级（Lv.10）特权**：节日与三餐动作无视时间限制随时可演（默认开、托盘可关、满级才显示）——`docs/batches/B35-pet-gallery-ui.md`
...
- 图鉴卡墙「悠闲哼歌」「摇扇纳凉」两张卡视频位黑框（同名段在分类组与常驻区并存时播放调度以段名为键互覆——`docs/batches/B35-pet-gallery-ui.md` §5.3）
- 托盘「满级特权 · 无视时间限制」开关对未满级用户可见造成误导（改为满级才显示，等级跨档自动刷新——同上 §5.4）
- 图鉴卡墙滚到底部时部分卡黑框（可见卡超过 8 张播放上限后，待命卡连首帧都不加载——改为入视即载首帧、播放仍限 8 路——同上 §5.5）
...
（2026-09-19/20 批次核销同步：条目源 = 各批 §6；B08–B26 历史回溯补写见 `docs/TODO.md` **T54**）
